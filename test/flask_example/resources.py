...
class DateTimeResource(Resource):
  ...
  def render_serializable(self, instance, context):
    ...
    if not isinstance(instance, DateTimeModel):
      """This check is here for illustration purposes, but isn't strictly
      required, as long as you make sure you send the correct objects to their
      correct serializers."""
      raise ValueError(("You broke the test app by trying to render something "
        "other than a `datetime.datetime` to the DateTimeResource serializer."))
